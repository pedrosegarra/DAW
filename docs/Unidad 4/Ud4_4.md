...
Hay tres tipos de usuarios dentro del servicio FTP:

- **Usuario anónimo**: usuario que no tiene contraseña de validación y da acceso público al servicio. Por ejemplo, el usuario "ftp" creado en un servidor vsftpd (Very Secure FTP Daemon) generalmente se utiliza para gestionar la operación del servidor FTP y, en algunos casos, para proporcionar acceso anónimo a los usuarios. Esto significa que los usuarios pueden conectarse al servidor FTP utilizando "anonymous" o "ftp" como nombre de usuario y una dirección de correo electrónico como contraseña. El acceso anónimo generalmente se utiliza para permitir a los usuarios descargar archivos públicos sin necesidad de una cuenta de usuario. El uso de este usuario deberá estar habilitado en el archivo de configuración vsftpd.conf con la opción anonymous_enable=YES.
- **Usuario del sistema**: usuario propio del sistema donde hay el servicio FTP y que accede a su directorio personal.
- **Usuarios virtuales**: este tipo de usuarios deriva de los usuarios de sistema. La diferencia viene dada porque estos no son dependientes del sistema sino del servidor FTP directamente.  Al no estar creados en el sistema operativo la validación de estos usuarios puede realizarse sobre un fichero de texto, una base de datos como Mysql o un servicio de directorio como LDAP.
