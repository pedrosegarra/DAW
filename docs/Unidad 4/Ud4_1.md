...
Estos modos se refieren a la forma en que se establecen las conexiones de datos entre el cliente y el servidor. En resumen la diferencia entre el modo pasivo FTP y el modo activo radica en el papel del servidor: en el modo activo, el servidor inicia la conexión. En el modo pasivo, el servidor deja que el cliente establezca la conexión y se limita a confirmarla.
...
- El servidor conecta desde el puerto 20 al cliente por el puerto de datos especificado por parte del cliente, que es el puerto N+1.
...
Para __controlar el cortafuego en el servidor FTP en modo pasivo__ abriremos los puertos siguientes:
