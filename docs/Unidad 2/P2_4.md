...
De la práctica anterior, dentro del directorio `/etc/nginx/sites-available` ya debéis tener el archivo de configuración llamado `practica2_2`. Es precisamente aquí donde realizaremos la configuración para que el acceso al sitio web se realice mediante SSL (HTTPS). 
...
+ Si accedéis ahora a `https://practica2_2` os debería saltar un aviso de seguridad debido a que nuestro certificado es autofirmado, como comentábamos anteriormente. 
...
Si ahora intentáis acceder a **`http://practica2_2`**, ¿deberíais poder acceder? Comprobadlo y describid qué pasa y por qué. 
...
Necesitamos añadir un bloque “server” adicional y separado del otro, al archivo de configuración de “practica2_2”. Algo así: 
...
server {
        ...
        return 301 https://practica2_2$request_uri;
}
```
...
Con esta configuración le estamos diciendo que: 
  ...
  + Que el nombre al que responderá el servidor/sitio web es `practica2_2`` 
  ...
  + Cuando se recibe una petición con las dos condiciones anteriores, se devuelve un código HTTP 301: 
    ...
    !!!Note "Aclaración" 
        Es decir, lo que estamos haciendo es que cuando se reciba una petición HTTP (puerto 80) en `http://practica2_2`, se redirija a `https://practica2_2` (HTTPS) 
